...
export function recomputeVisibility(ctx: Ctx, state: GameState, player: PlayerId): void {
  ...
  const mark = (other: PlayerId) => {
    ...
    if (!state.relations[player][other].met) state.relations[player][other].firstContactTurn = state.turn;
    if (!state.relations[other][player].met) state.relations[other][player].firstContactTurn = state.turn;
